...
export function traceShape(kind) {
  ...
  switch (kind) {
    ...
    // ---------- expansion: 40+ options per Trace Rush mode ----------
    case "nonagon": return polyPts(cx, cy, 9, 175); case "star_10": return starPts(cx, cy, 10, 175, 110); case "star_12": return starPts(cx, cy, 12, 175, 120);
    case "plus_sign": { const a = 50; return [[cx - a, cy - 170], [cx + a, cy - 170], [cx + a, cy - a], [cx + 170, cy - a], [cx + 170, cy + a], [cx + a, cy + a], [cx + a, cy + 170], [cx - a, cy + 170], [cx - a, cy + a], [cx - 170, cy + a], [cx - 170, cy - a], [cx - a, cy - a], [cx - a, cy - 170]]; }
    case "hourglass_shape": return [[cx - 110, cy - 170], [cx + 110, cy - 170], [cx, cy], [cx + 110, cy + 170], [cx - 110, cy + 170], [cx, cy], [cx - 110, cy - 170]];
    case "trapezoid": return [[cx - 160, cy + 120], [cx + 160, cy + 120], [cx + 80, cy - 120], [cx - 80, cy - 120], [cx - 160, cy + 120]];
    case "parallelogram": return [[cx - 160, cy + 120], [cx + 80, cy + 120], [cx + 160, cy - 120], [cx - 80, cy - 120], [cx - 160, cy + 120]];
    case "arrow_up": return [[cx, cy - 170], [cx + 90, cy - 60], [cx + 40, cy - 60], [cx + 40, cy + 170], [cx - 40, cy + 170], [cx - 40, cy - 60], [cx - 90, cy - 60], [cx, cy - 170]];
    case "arrow_down": return [[cx, cy + 170], [cx + 90, cy + 60], [cx + 40, cy + 60], [cx + 40, cy - 170], [cx - 40, cy - 170], [cx - 40, cy + 60], [cx - 90, cy + 60], [cx, cy + 170]];
    case "chevron": return [[cx - 140, cy - 100], [cx, cy + 80], [cx + 140, cy - 100], [cx + 140, cy - 40], [cx, cy + 140], [cx - 140, cy - 40], [cx - 140, cy - 100]];
    case "lens": return [...ellipsePts(cx - 40, cy, 120, 150, 40), ...ellipsePts(cx + 40, cy, 120, 150, 40)];
    case "yin_yang": { for (let i = 0; i <= 50; i++) { const a = i / 50 * Math.PI; pts.push([cx + Math.sin(a) * 85, cy - 85 + Math.cos(a) * 85]); } for (let i = 0; i <= 50; i++) { const a = i / 50 * Math.PI; pts.push([cx - Math.sin(a) * 85, cy + 85 - Math.cos(a) * 85]); } return pts; }
    case "keyhole": return [...ellipsePts(cx, cy - 60, 60, 60, 30), [cx + 20, cy], [cx + 30, cy + 150], [cx - 30, cy + 150], [cx - 20, cy]];
    case "quatrefoil": { for (let i = 0; i <= 160; i++) { const a = i / 160 * Math.PI * 2; const r = 140 * Math.abs(Math.cos(2 * a)); pts.push([cx + Math.cos(a) * r, cy + Math.sin(a) * r]); } return pts; }
    case "umbrella": return [...ellipsePts(cx, cy - 30, 150, 90, 24).filter((_, i) => i <= 12), ...linePts([cx, cy - 30], [cx, cy + 150]), ...linePts([cx, cy + 150], [cx + 35, cy + 150]), ...linePts([cx + 35, cy + 150], [cx + 35, cy + 115])];
    case "key": return [...ellipsePts(cx - 100, cy - 60, 50, 50, 24), ...linePts([cx - 50, cy - 60], [cx + 130, cy - 60]), ...linePts([cx + 130, cy - 60], [cx + 130, cy - 20]), ...linePts([cx + 100, cy - 60], [cx + 100, cy - 10]), ...linePts([cx + 70, cy - 60], [cx + 70, cy - 30])];
    case "bell": return [[cx - 90, cy + 80], [cx - 90, cy + 20], [cx - 60, cy - 100], [cx, cy - 140], [cx + 60, cy - 100], [cx + 90, cy + 20], [cx + 90, cy + 80], [cx - 90, cy + 80], ...ellipsePts(cx, cy + 100, 20, 10, 10)];
    case "kite": return [[cx, cy - 170], [cx + 90, cy - 20], [cx, cy + 60], [cx - 90, cy - 20], [cx, cy - 170], ...linePts([cx, cy + 60], [cx, cy + 170])];
    case "top_hat": return [[cx - 140, cy + 80], [cx + 140, cy + 80], [cx + 140, cy + 40], [cx + 70, cy + 40], [cx + 70, cy - 140], [cx - 70, cy - 140], [cx - 70, cy + 40], [cx - 140, cy + 40], [cx - 140, cy + 80]];
    case "teapot": return [...ellipsePts(cx, cy + 30, 120, 80, 30), [cx + 120, cy + 10], [cx + 170, cy - 40], [cx + 130, cy - 20], ...ellipsePts(cx, cy - 70, 30, 15, 10), [cx - 120, cy + 10], [cx - 160, cy + 40], [cx - 160, cy + 80], [cx - 120, cy + 60]];
    case "boat": return [[cx - 150, cy + 100], [cx + 150, cy + 100], [cx + 100, cy + 150], [cx - 100, cy + 150], [cx - 150, cy + 100], ...linePts([cx, cy + 100], [cx, cy - 170]), [cx, cy - 170], [cx + 90, cy + 80], [cx, cy + 80]];
    case "bow": return [[cx, cy], [cx - 140, cy - 80], [cx - 140, cy + 80], [cx, cy], [cx + 140, cy - 80], [cx + 140, cy + 80], [cx, cy], ...ellipsePts(cx, cy, 20, 20, 12)];
    case "ribbon": return [[cx - 160, cy - 40], [cx - 100, cy], [cx - 160, cy + 40], [cx - 100, cy - 20], [cx + 100, cy - 20], [cx + 160, cy + 40], [cx + 100, cy], [cx + 160, cy - 40], [cx + 100, cy - 20], [cx - 100, cy - 20]];
    case "mug": return [[cx - 90, cy - 100], [cx - 90, cy + 120], [cx + 70, cy + 120], [cx + 70, cy - 100], [cx - 90, cy - 100], ...ellipsePts(cx + 100, cy + 10, 40, 60, 16)];
    case "candle": return [[cx - 40, cy + 170], [cx - 40, cy - 60], [cx + 40, cy - 60], [cx + 40, cy + 170], [cx - 40, cy + 170], ...ellipsePts(cx, cy - 110, 20, 40, 14)];
    case "lantern": return [[cx - 70, cy - 100], [cx + 70, cy - 100], [cx + 70, cy + 100], [cx - 70, cy + 100], [cx - 70, cy - 100], ...linePts([cx, cy - 100], [cx, cy - 150]), ...linePts([cx, cy + 100], [cx, cy + 150]), [cx - 40, cy - 100], [cx - 40, cy + 100], [cx + 40, cy - 100], [cx + 40, cy + 100]];
    case "scissors": return [...ellipsePts(cx - 80, cy + 100, 30, 30, 14), ...linePts([cx - 80, cy + 100], [cx + 120, cy - 140]), ...ellipsePts(cx - 80, cy - 100, 30, 30, 14), ...linePts([cx - 80, cy - 100], [cx + 120, cy + 140])];
    case "envelope": return [[cx - 160, cy - 100], [cx + 160, cy - 100], [cx + 160, cy + 100], [cx - 160, cy + 100], [cx - 160, cy - 100], [cx, cy + 10], [cx + 160, cy - 100], [cx - 160, cy - 100], [cx, cy + 10], [cx + 160, cy + 100]];
    case "fractal_branch": { for (let k = 0; k < 5; k++) { const ang = -Math.PI / 2 + (k - 2) * 0.5; for (let i = 0; i <= 30; i++) { const t = i / 30; pts.push([cx + Math.cos(ang) * t * 170 + Math.sin(t * 10 + k) * 10, cy + Math.sin(ang) * t * 170 + 50]); } } return pts; }
    case "static_burst": { for (let i = 0; i <= 90; i++) { const a = i / 90 * Math.PI * 2; const r = 40 + 130 * Math.abs(Math.sin(i * 2.7)); pts.push([cx + Math.cos(a) * r, cy + Math.sin(a) * r]); } return pts; }
    case "thorn_ring": { const n = 20; for (let i = 0; i <= n * 2; i++) { const a = i / (n * 2) * Math.PI * 2; const r = i % 2 === 0 ? 170 : 110; pts.push([cx + Math.cos(a) * r, cy + Math.sin(a) * r]); } return pts; }
    case "knotwork": { for (let i = 0; i <= 140; i++) { const t = i / 140 * Math.PI * 4; pts.push([cx + Math.sin(t) * 140 + Math.sin(t * 3) * 30, cy + Math.cos(t * 1.5) * 140 + Math.cos(t * 2) * 30]); } return pts; }
    case "storm_front": { for (let i = 0; i <= 40; i++) { const t = i / 40; pts.push([cx - 170 + t * 340, cy + Math.sin(t * 30) * 60 + Math.cos(t * 13) * 40]); } return pts; }
    case "ripple_field": { for (let ring = 0; ring < 3; ring++) { for (let i = 0; i <= 60; i++) { const a = i / 60 * Math.PI * 2; const r = (60 + ring * 50) + 15 * Math.sin(a * 6); pts.push([cx + Math.cos(a) * r, cy + Math.sin(a) * r]); } } return pts; }
    case "crack_glass": { for (let k = 0; k < 7; k++) { const ang = k / 7 * Math.PI * 2; for (let i = 0; i <= 15; i++) { const t = i / 15; pts.push([cx + Math.cos(ang + Math.sin(i) * 0.3) * t * 170, cy + Math.sin(ang + Math.sin(i) * 0.3) * t * 170]); } } return pts; }
    case "thread_snarl": { for (let i = 0; i <= 160; i++) { const t = i / 160 * Math.PI * 8; const r = 60 + 40 * Math.sin(t * 0.6); pts.push([cx + Math.cos(t) * r + Math.sin(t * 2.3) * 20, cy + Math.sin(t) * r + Math.cos(t * 1.7) * 20]); } return pts; }
    case "bramble": { for (let k = 0; k < 6; k++) { const ang = k / 6 * Math.PI * 2 + 0.3; for (let i = 0; i <= 20; i++) { const t = i / 20; pts.push([cx + Math.cos(ang) * t * 150 + Math.cos(t * 15) * 15, cy + Math.sin(ang) * t * 150 + Math.sin(t * 15) * 15]); } } return pts; }
    case "corkscrew": { for (let i = 0; i <= 150; i++) { const t = i / 150 * Math.PI * 8; const r = 170 * (1 - i / 150); pts.push([cx + Math.cos(t) * r, cy + Math.sin(t) * r]); } return pts; }
    case "wormhole": { for (let ring = 0; ring < 6; ring++) { const rr = 170 - ring * 28; for (let i = 0; i <= 40; i++) { const a = i / 40 * Math.PI * 2; pts.push([cx + Math.cos(a) * rr, cy + Math.sin(a) * rr * 0.4]); } } return pts; }
    case "shatter": { for (let k = 0; k < 9; k++) { const ang = k / 9 * Math.PI * 2; pts.push([cx, cy], [cx + Math.cos(ang) * 170 + Math.sin(k) * 20, cy + Math.sin(ang) * 170 + Math.cos(k) * 20]); } return pts; }
    case "char_spider": { pts = [...ellipsePts(cx, cy, 70, 50, 20)]; for (let k = 0; k < 8; k++) { const ang = k / 8 * Math.PI * 2; pts.push(...linePts([cx, cy], [cx + Math.cos(ang) * 160, cy + Math.sin(ang) * 160])); } return pts; }
    case "char_crab": return [...ellipsePts(cx, cy, 100, 60, 24), ...linePts([cx - 100, cy - 20], [cx - 160, cy - 80]), ...linePts([cx - 160, cy - 80], [cx - 130, cy - 60]), ...linePts([cx + 100, cy - 20], [cx + 160, cy - 80]), ...linePts([cx + 160, cy - 80], [cx + 130, cy - 60]), ...linePts([cx - 70, cy + 40], [cx - 110, cy + 90]), ...linePts([cx + 70, cy + 40], [cx + 110, cy + 90])];
    case "char_octopus": { pts = [...ellipsePts(cx, cy - 60, 90, 70, 24)]; for (let k = 0; k < 6; k++) { const ang = Math.PI * 0.3 + k / 6 * Math.PI * 0.7; for (let i = 0; i <= 15; i++) { const t = i / 15; pts.push([cx + Math.cos(ang) * t * 150 + (k - 3) * 10, cy + 40 + t * 140 + Math.sin(t * 8 + k) * 15]); } } return pts; }
    case "char_bear": return [[cx - 90, cy + 140], [cx - 110, cy + 40], [cx - 90, cy - 60], [cx - 40, cy - 110], [cx, cy - 120], [cx + 40, cy - 110], [cx + 90, cy - 60], [cx + 110, cy + 40], [cx + 90, cy + 140], [cx + 40, cy + 100], [cx, cy + 110], [cx - 40, cy + 100], ...ellipsePts(cx - 60, cy - 120, 20, 20, 10), ...ellipsePts(cx + 60, cy - 120, 20, 20, 10)];
    case "char_deer": return [[cx - 70, cy + 130], [cx - 100, cy + 50], [cx - 90, cy - 40], [cx - 50, cy - 90], [cx, cy - 100], [cx + 50, cy - 90], [cx + 90, cy - 40], [cx + 100, cy + 50], [cx + 70, cy + 130], ...linePts([cx - 30, cy - 90], [cx - 70, cy - 160]), ...linePts([cx - 70, cy - 160], [cx - 50, cy - 150]), ...linePts([cx + 30, cy - 90], [cx + 70, cy - 160]), ...linePts([cx + 70, cy - 160], [cx + 50, cy - 150])];
    case "char_lion": return [...ellipsePts(cx, cy - 20, 140, 140, 30), ...ellipsePts(cx, cy - 20, 70, 70, 20)];
    case "char_penguin": return [[cx, cy - 140], [cx + 60, cy - 100], [cx + 80, cy + 20], [cx + 50, cy + 140], [cx - 50, cy + 140], [cx - 80, cy + 20], [cx - 60, cy - 100], [cx, cy - 140], ...ellipsePts(cx, cy - 10, 35, 60, 16)];
    case "char_squirrel": return [[cx - 40, cy + 120], [cx - 60, cy + 40], [cx - 40, cy - 40], [cx, cy - 70], [cx + 30, cy - 40], ...ellipsePts(cx + 90, cy - 40, 70, 110, 24)];
    case "char_turtle": return [...ellipsePts(cx, cy, 130, 90, 30), [cx + 130, cy - 10], [cx + 180, cy - 30], [cx + 160, cy], [cx + 180, cy + 30], [cx + 130, cy + 10]];
    case "char_witch": return [[cx, cy - 180], [cx + 50, cy - 60], [cx - 90, cy - 60], [cx, cy - 180], [cx - 70, cy - 40], [cx + 70, cy - 40], [cx + 100, cy + 140], [cx - 100, cy + 140], [cx - 70, cy - 40]];
    case "char_pirate": return [...ellipsePts(cx, cy - 20, 90, 100, 24), [cx - 100, cy - 60], [cx + 100, cy - 60], [cx, cy - 140], [cx - 100, cy - 60], ...linePts([cx - 40, cy - 20], [cx, cy - 10])];
    case "char_samurai": return [[cx - 90, cy + 120], [cx - 100, cy + 20], [cx - 70, cy - 60], [cx, cy - 120], [cx + 70, cy - 60], [cx + 100, cy + 20], [cx + 90, cy + 120], ...linePts([cx - 20, cy - 120], [cx - 50, cy - 190]), ...linePts([cx + 20, cy - 120], [cx + 50, cy - 190])];
    case "char_astronaut": return [...ellipsePts(cx, cy - 70, 80, 80, 24), [cx - 60, cy - 10], [cx - 80, cy + 140], [cx + 80, cy + 140], [cx + 60, cy - 10]];
    case "char_mermaid": return [...ellipsePts(cx, cy - 90, 50, 60, 20), [cx - 30, cy - 30], [cx - 60, cy + 60], [cx - 20, cy + 40], [cx, cy + 170], [cx + 20, cy + 40], [cx + 60, cy + 60], [cx + 30, cy - 30]];
