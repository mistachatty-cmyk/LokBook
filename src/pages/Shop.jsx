...
export const WIP_CATEGORIES = {
  stickerPack: "Sticker packs aren't available in Studio yet.",
  postExport: "Extra export formats aren't implemented yet.",
...
export default function Shop({ccTier,say,modules=[],onBuyModule,loks,lokPass,kids,uiTheme,ownedThemes,effect,ownedEffects,sky,ownedSkies=[],onSky,animFx,ownedAnimFx=[],onAnimFx,fontPack,onFontPack,cursorPack,onCursorPack,musicPack,onMusicPack,stickerPack,onStickerPack,postExport,onPostExport,cosmetics,owned,onBuyCosmetic,setKids,onBuyPass,onTheme,onEffect,onCc,mythicOwned,mythicEquipped,onBuyMythic,dailyOwned,weeklyOwned,celebrationStyle,onCelebrationStyle}){
  ...
  return(<div className="mt-4">
    ...
    {activeTab==="fonts"&&(<Section title="Font Packs" sub="Change the app font style."><div className="grid grid-cols-2 gap-2">{FONT_PACKS.filter(f=>showAll||f.id!=="default").map(f=>{const own=has("fontPack",f.id);const e2=eq("fontPack",f.id);return(<ShopItem key={f.id} owned={own} equipped={e2} price={f.price} onClick={()=>buy("fontPack",f)}><div className="font-bold text-sm">{f.name}</div>{f.desc&&<div className="text-[10px] opacity-70">{f.desc}</div>}</ShopItem>);})}</div></Section>)}
    {activeTab==="stickers"&&(<Section title="Sticker Packs" sub="Unlock themed sticker sets for your Studio."><div className="grid grid-cols-2 gap-2">{STICKER_PACKS.filter(s=>showAll||s.id!=="emoji").map(s=>{const own=has("stickerPack",s.id);const e2=eq("stickerPack",s.id);return(<ShopItem key={s.id} owned={own} equipped={e2} price={s.price} wip={!!WIP_CATEGORIES.stickerPack} onClick={()=>buy("stickerPack",s)}><div className="font-bold text-sm">{s.name}</div><div className="text-[10px] opacity-70">{s.stickers.slice(0,4).join(" ")}{s.stickers.length>4?" +"+s.stickers.length:""}</div></ShopItem>);})}</div></Section>)}
    {activeTab==="export"&&(<Section title="Export Formats" sub="Extra export options for your flips."><div className="grid grid-cols-2 gap-2">{POST_EXPORTS.filter(e=>showAll||e.id!=="png").map(e=>{const own=has("postExport",e.id);const e2=eq("postExport",e.id);return(<ShopItem key={e.id} owned={own} equipped={e2} price={e.price} wip={!!WIP_CATEGORIES.postExport} onClick={()=>buy("postExport",e)}><div className="font-bold text-sm">{e.name}{e.soon&&<span className="ml-1 text-[9px] opacity-50">soon</span>}</div>{e.desc&&<div className="text-[10px] opacity-70">{e.desc}</div>}</ShopItem>);})}</div></Section>)}
    {activeTab==="cosmetic"&&(<>
