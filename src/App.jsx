...
function Feed({posts,bookmarks,following,feedMode,setFeedMode,myHandle="",onFeatureOpen,cosmetics={},daily,streak,dailyClaimed,flipOfDay,onLine,onClaimDaily,onOpen,onVote,onLok,onBookmark,say,moodFilter,setMoodFilter,moodTags,reportedPosts,onReport,onEcho,onArtist,flair="",onPullRefresh,music}){
  ...
  return(<div onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
    ...
    {feedMode==="artists"?<NewArtists onArtist={onArtist} myHandle={myHandle} say={say}/>:list.length===0?(
      ...
      <div className="mt-3 flex flex-col gap-5">
        {list.map((p,i)=>(<div key={p.id} ref={el=>cardRefs.current[i]=el} data-idx={i}><FeedCard p={p} live={i===active} marked={bookmarks.includes(p.id)} loked={following.includes(p.author||"moss.ink")} cosmetics={cosmetics} onOpen={onOpen} onVote={onVote} onLok={onLok} onBookmark={onBookmark} moodTags={moodTags} onReport={onReport} onEcho={onEcho} onArtist={onArtist} flair={flair} music={music}/></div>))}
