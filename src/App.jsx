...
function NewStudioUI({ownedTiers,ccTier,onPublish,say,kids,dailyPrompt,animFx,modules=[],legacyBrushes,setLegacyBrushes,frames,setFrames,frameDurations,setFrameDurations,title,setTitle,draftImg,setDraftImg,authorName,tutorialGhostFrames,paper="plain",cursorPack="default",postExport="png",mythicEquipped,stickers,onStickersChange,pendingSticker,onStickerPlaced}){
  ...
  return(<div className="mt-4">
    ...
    {frames.length>0&&(<div style={{animation:"lokrise .3s ease"}}>
      ...
      {showMusicPicker&&(<div className="fixed inset-0 z-50 flex items-end justify-center" style={{background:"rgba(0,0,0,.35)"}} onClick={()=>setShowMusicPicker(false)}><div className="w-full rounded-t-3xl p-5 overflow-y-auto overscroll-contain" style={{maxWidth:560,maxHeight:"70vh",background:T.card,border:`3px solid ${T.ink}`,animation:"lokrise .25s ease"}} onClick={e=>e.stopPropagation()}><div className="flex items-center justify-between mb-3"><div className="lok-display font-extrabold">🎵 Select a track</div><button onClick={()=>setShowMusicPicker(false)} className="lok-btn px-3 py-1 rounded-lg font-bold" style={{border:`2.5px solid ${T.ink}`}}>✕</button></div><div className="text-xs opacity-70 mb-3">Pick a track from your library to play with this post when viewed.</div><div className="space-y-2">{/* Music tracks would go here */}<div className="text-xs opacity-50 text-center py-4">No music library loaded yet.<br/>Open Music player (♪) to add tracks.</div></div></div></div>)}
