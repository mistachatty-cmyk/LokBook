...
function Viewer({posts,index,bookmarks,cosmetics={},onBookmark,onClose,onNav,onVote,onReact,onViewed,onShare,onDelete,onRename,myName="",onRemix,onPatch,onEditInStudio}){
...
function Studio({ownedTiers,ccTier,onPublish,say,kids,dailyPrompt,animFx,modules=[],legacyBrushes,setLegacyBrushes,authorName,paper="plain",cursorPack="default",mythicEquipped,stickers,onStickersChange,pendingSticker,onStickerPlaced}){
  ...
  const capture=()=>{const url=easel.current.composite(frames.length);setFrames(f=>[...f,url]);setFrameDurations(d=>[...d,paceMs]);onStickersChange&&onStickersChange([]);setJustCap(true);setTimeout(()=>setJustCap(false),360);say(`Page ${frames.length+1} captured`);};
...
function NewStudioUI({ownedTiers,ccTier,onPublish,say,kids,dailyPrompt,animFx,modules=[],legacyBrushes,setLegacyBrushes,frames,setFrames,frameDurations,setFrameDurations,title,setTitle,draftImg,setDraftImg,authorName,tutorialGhostFrames,paper="plain",cursorPack="default",postExport="png",mythicEquipped,stickers,onStickersChange,pendingSticker,onStickerPlaced,music,geoData}){
  ...
  const capture=()=>{const url=easel.current.composite(frames.length);setFrames(f=>[...f,url]);setFrameDurations(d=>[...d,paceMs]);setFrameLabels(l=>[...l,""]);onStickersChange&&onStickersChange([]);setJustCap(true);setTimeout(()=>setJustCap(false),360);if(autoAdvance){setTimeout(()=>{const url2=easel.current.composite(frames.length+1);setFrames(f=>[...f,url2]);setFrameDurations(d=>[...d,paceMs]);setFrameLabels(l=>[...l,""]);say(`Pages ${frames.length+1}-${frames.length+2} captured`);},100);}else say(`Page ${frames.length+1} captured`);};
...
function Battle({ownedTiers,ccTier,wins,bigBattleOwned,kids,phase,lillok,customLilLok,onResult,onUnlockBig,onPublish,onLine,blip,hap,say,animFx,authorName,modules=[],paper="plain",cursorPack="default"}){
  ...
  const submitDuelNow=async()=>{if(!easel.current||!duel)return;const url=easel.current.composite();setMyArt(url);await submitDuelArt(duel.id,isPlayer1Ref.current,url);setPstate("duel_wait");};
  ...
  if(pstate==="draw")return(<div className="mt-3" style={{animation:shake&&!reduceMotion?"lokshake .9s":"none"}}>
    ...
    <div className="mt-2 relative">
      <Easel ref={easel} maxLayers={tier} ccTier={ccTier} modules={modules} paper={paper} cursorPack={cursorPack} onStroke={()=>(strokes.current+=1)} animFx={animFx}/>
  ...
  if(pstate==="duel_draw")return(<div className="mt-3">
    ...
    <div className="mt-2"><Easel ref={easel} maxLayers={1} ccTier={ccTier} modules={modules} paper={paper} cursorPack={cursorPack} onStroke={()=>(strokes.current+=1)} animFx={animFx}/></div>
    ...
    {showSettings&&(<div className="fixed inset-0 z-50 flex items-end justify-center" style={{background:"rgba(0,0,0,.35)"}} onClick={()=>setShowSettings(false)}>
      ...
      <div className="w-full rounded-t-3xl p-5 overflow-y-auto overscroll-contain" style={{maxWidth:560,maxHeight:"min(85vh, 85dvh)",background:T.card,border:`3px solid ${T.ink}`,animation:"lokrise .25s ease",WebkitOverflowScrolling:"touch"}} onClick={e=>e.stopPropagation()}>
        ...
        <div className="p-3 rounded-2xl mb-2" style={{border:`2px solid ${T.shadow}`,background:T.paper}}>
          ...
          <label className="mt-2 flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={!!featureFlags.enableGyroscope} onChange={e=>{const enabled=e.target.checked;if(enabled)onRequestGyroPermission&&onRequestGyroPermission();const newFlags={enableGyroscope:enabled,gyroLoadScreen:enabled,gyroFab:enabled,gyroLilLokPanel:enabled,gyroWorldMap:enabled,gyroChestOpen:enabled,gyroGalleryCards:enabled};setFeatureFlags(f=>({...f,...newFlags}));store.set("lok:flags",{...featureFlags,...newFlags});}} style={{accentColor:T.accent}}/> 🎮 Gyroscope (tilt interaction)</label>
          ...
          <div className="grid grid-cols-3 gap-1.5">{[["","Auto"],["rain","Rain"],["fog","Fog"],["aurora","Aurora"],["dust","Dust"]].map(([id,label])=>(
            <button key={id} onClick={()=>{setWeatherOverride(id);setWeatherOverrideLocal(id);}} aria-pressed={(weatherOverride||"")==id} className="lok-btn py-1.5 rounded-xl text-xs font-extrabold" style={{border:`2.5px solid ${(weatherOverride||"")==id?T.accent:T.ink}`,background:(weatherOverride||"")==id?T.ink:T.card,color:(weatherOverride||"")==id?T.paper:T.ink}}>{label}</button>))}</div>
        </div>
...
export default function LokApp(){
  ...
  useEffect(()=>{
    ...
    if(rewardType==="loks"){
      ...
      reward={type:"cosmetic",category:cat,itemName:"Mystery cosmetic"};
  ...
  // iOS only grants DeviceMotionEvent.requestPermission() when it's called
  ...
  return(<ThemeCtx.Provider value={T}>
    ...
    <div className={`min-h-screen w-full lok-motion-${featureFlags.lokMotion||"subtle"} ${featureFlags.compactUi ? "lok-compact" : ""}`} style={{background:T.paper,color:T.ink,fontFamily:resolveFont(fontPack),...(isUnlocked("night_shift",level)&&nightShiftAmount()>0.02?{filter:`brightness(${1-nightShiftAmount()*0.16}) saturate(${1-nightShiftAmount()*0.22}) hue-rotate(${-nightShiftAmount()*8}deg)`,transition:"filter 4s linear"}:{}),animation:effect==="quake"&&!reduceMotion?"lokquake 6s infinite":"none"}}>
      ...
      {!focusMode && !showLilLok&&(<div className="fixed z-42" style={{right:14,bottom:116, transition: 'bottom 0.3s ease'}}>
        ...
        {openingChest&&<ChestOpen chestType={openingChest.type} reward={chestReward} onComplete={()=>setOpeningChest(null)} gyroMotion={featureFlags.enableGyroscope&&featureFlags.gyroChestOpen?gyroMotion:{gamma:0,beta:0,alpha:0}}/>}
