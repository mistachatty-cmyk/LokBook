...
import {
  ...
  RARITY, MYTHIC_ITEMS, DAILY_ITEMS, WEEKLY_ITEMS, getDailyRotation, getWeeklyRotation, fromDbPost
} from "./constants.jsx";
...
function Feed({posts,bookmarks,following,feedMode,setFeedMode,cosmetics={},daily,streak,dailyClaimed,flipOfDay,onLine,onClaimDaily,onOpen,onVote,onLok,onBookmark,say,moodFilter,setMoodFilter,moodTags,reportedPosts,onReport,onEcho}){
  ...
  return(<div>
    ...
    {list.length===0?(
      ...
      <div ref={wrapRef} onScroll={onScroll} className="mt-3 -mx-4" style={{height:"calc(100dvh - 300px)",minHeight:360,overflowY:"scroll",scrollSnapType:"y mandatory"}}>
        {list.map((p,i)=>(<FeedCard key={p.id} p={p} live={i===active} marked={bookmarks.includes(p.id)} loked={loked} cosmetics={cosmetics} onOpen={onOpen} onVote={onVote} onLok={onLok} onBookmark={onBookmark} moodTags={moodTags} onReport={onReport} onEcho={onEcho}/>))}
      </div>
    )}</>)}
  </div>);
}

function FeedCard({p,live,marked,loked,cosmetics={},onOpen,onVote,onLok,onBookmark,moodTags,onReport,onEcho}){
...
export default function LokApp(){
  ...
  useEffect(()=>{if(!ready)return;const picks=pickAmbientPosts(botPosted,2);if(!picks.length)return;setBotPosted(b=>[...b,...picks.map(p=>p.key)].slice(-400));picks.forEach((pk,i)=>{setTimeout(()=>{try{const post=generateBotPost(pk.bot,pk.seed);if(post)setPosts(ps=>ps.some(x=>x.id===post.id)?ps:[...ps.slice(0,3),post,...ps.slice(3)]);}catch(e){console.warn("botArt",e);}},1600+i*900);});},[ready,botPosted]);
  ...
  useEffect(()=>{(async()=>{
    ...
    (async()=>{try{const{data,error}=await supabase.from("lok_posts").select("id,title,frames,pace_ms,mode,style,author,votes,views").order("votes",{ascending:false}).limit(6);if(!error&&data){const dbPosts=data.map(fromDbPost).filter(Boolean).map(p=>({...p,voted:false,viewed:false}));setPosts(ps=>{const existing=new Set(ps.map(x=>x.id));return[...ps,...dbPosts.filter(dp=>!existing.has(dp.id))];});}}catch{}})().finally(()=>setReady(true));
