...
function LokApp(){
  ...
  const[account,setAccount]=useState(null);const[tips,setTips]=useState({});const[artistView,setArtistView]=useState(null);const[botPosted,setBotPosted]=useState([]);const[online,setOnline]=useState(typeof navigator!=="undefined"?navigator.onLine:true);
  ...
  useEffect(()=>{if(!ready)return;const t=setTimeout(()=>{const blob={loks,lokPass,uiTheme,ownedThemes,effect,ownedEffects,sky,ownedSkies,studioModules,bigBattleOwned,wins,offlineBonusDay,tides,eventClaimed,profile,bookmarks,following,kids,customLilLok,cosmetics,owned,onboarded,sound,xp,flair,daily,quests,questsCompleted,totalEarned,traceHinted,pace,speed,soundLab,soundQueue,founder,totalSpent,fodHistory,account,tips,compactDensity,botPosted,lillok:{...lillok,lastSeen:Date.now()}};store.set(SAVE_KEY,blob);if(account&&Date.now()-lastCloudPush.current>15000){lastCloudPush.current=Date.now();lokApi.pushSave(account,blob,auth.getUserId());}},400);return()=>clearTimeout(t);},[ready,loks,lokPass,uiTheme,ownedThemes,effect,ownedEffects,sky,ownedSkies,studioModules,bigBattleOwned,wins,offlineBonusDay,tides,eventClaimed,profile,bookmarks,following,kids,customLilLok,cosmetics,owned,onboarded,sound,xp,flair,daily,quests,questsCompleted,totalEarned,traceHinted,pace,speed,soundLab,soundQueue,founder,totalSpent,fodHistory,account,tips,compactDensity,botPosted,lillok]);
