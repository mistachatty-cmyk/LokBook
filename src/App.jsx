...
import {
  ...
  AVATAR_ACCENTS, PAPERS, LILLOK_GEAR, STUDIO_UPGRADES, blotBorderStyle, makeQuests, founderSignup,
  lokApi, toDbPost, fromDbPost
} from "./constants.jsx";
import { paperBase, risoCircle, drawBounce, drawBloom, drawNight, drawOrbit, drawWalk, drawRain, drawFish, drawBurst, drawWave, compressFrame, renderSequence, makeRng, makeDoodlePainter, renderDoodle, renderAvatar, polyPts, starPts, ellipsePts, linePts, traceShape, MiniDraw } from "./engine/draw.jsx";
...
function Feed({posts,bookmarks,following,feedMode,setFeedMode,cosmetics={},daily,streak,dailyClaimed,flipOfDay,onLine,onClaimDaily,onOpen,onVote,onLok,onBookmark,onArtist,say}){
  const T=useT();const[active,setActive]=useState(0);const wrapRef=useRef(null);
  const list=feedMode==="following"?posts.filter(p=>following.includes(p.author)):posts;
  ...
  return(<div>
    ...
    {list.length===0?(
      ...
      <div ref={wrapRef} onScroll={onScroll} className="mt-3 -mx-4" style={{height:"calc(100dvh - 300px)",minHeight:360,overflowY:"scroll",scrollSnapType:"y mandatory"}}>
        {list.map((p,i)=>(<FeedCard key={p.id} p={p} live={i===active} marked={bookmarks.includes(p.id)} following={following} cosmetics={cosmetics} onOpen={onOpen} onVote={onVote} onLok={onLok} onBookmark={onBookmark} onArtist={onArtist}/>))}
      </div>
    )}
  </div>);
}

function FeedCard({p,live,marked,following=[],cosmetics={},onOpen,onVote,onLok,onBookmark,onArtist}){
...
const Easel=forwardRef(function Easel({maxLayers,ccTier,onionFrames=[],onStroke},ref){
  ...
  return(<div>
    ...
    <div className="mt-1.5 flex items-center gap-1.5 overflow-x-auto pb-1" role="toolbar" aria-label="Tools">
      ...
      {ccTier&&<div className="flex items-center gap-1.5"><span className="text-xs font-bold opacity-60 shrink-0">pro</span>
...
function Studio({ownedTiers,ccTier,onPublish,say,kids,dailyPrompt}){
  ...
  const[tier,setTier]=useState(10);const[frames,setFrames]=useState(()=>{try{const d=localStorage.getItem("lok:draft:frames");return d?JSON.parse(d):[];}catch{return[];}});const[frameDurations,setFrameDurations]=useState(()=>{try{const d=localStorage.getItem("lok:draft:durations");return d?JSON.parse(d):[];}catch{return[];}});
...
function Shop({loks,lokPass,kids,uiTheme,ownedThemes,effect,ownedEffects,ownedTiers,ccTier,studioUpgrades=[],onStudioUpgrade,cosmetics,owned,onBuyCosmetic,setKids,onBuyPass,onTheme,onEffect,onTier,onCc,founder=false,onFounderJoin,say}){
  const T=useT();const[catTab,setCatTab]=useState("featured");const[fHandle,setFHandle]=useState("");const[fEmail,setFEmail]=useState("");const[fBusy,setFBusy]=useState(false);
  ...
  const cats=[["featured","Featured"],["themes","Skins"],["effects","Effects"],["cosmetic","Cosmetics"],["studio","Studio"],["paper","Paper"],["blot","Blot Shop"]];
  return(<div className="mt-4">
    ...
    {catTab==="studio"&&(<>
      ...
      <Section title="Studio upgrades"><div className="grid grid-cols-2 gap-2">{STUDIO_UPGRADES.map(u=>{const own=studioUpgrades.includes(u.id);return(<ShopItem key={u.id} owned={own} equipped={own} price={u.price} onClick={()=>onStudioUpgrade?.(u)}><div className="font-bold text-sm">{u.name}</div><div className="text-[10px] opacity-70">{u.desc}</div></ShopItem>);})}</div></Section>
    </>)}
  ...
  return(<div className="fixed inset-0 z-40 overflow-y-auto" style={{background:T.paper,color:T.ink,animation:"lokrise .25s ease"}}>
    <div className="mx-auto w-full px-4 pb-24" style={{maxWidth:560}}>
      <div className="sticky top-0 z-10 flex items-center gap-3 py-3" style={{background:T.paper,borderBottom:`3px solid ${T.ink}`}}>
        ...
        <img src={avatar} alt={name} className="w-11 h-11 rounded-full" style={{border:`2.5px solid ${T.ink}`}}/>
        <div className="min-w-0 flex-1"><div className="lok-display font-extrabold leading-tight truncate">{name}</div><div className="text-xs opacity-60">{theirs.length} {theirs.length===1?"flip":"flips"} · {theirs.reduce((a,p)=>a+(p.votes||0),0)} votes</div></div>
        <button onClick={()=>onLok(name)} aria-label={loked?"Already Lok'd":`Lok ${name}`} className="lok-btn px-3 py-1.5 rounded-full text-xs font-extrabold" style={{background:loked?T.card:T.accent,color:loked?T.ink:T.onAccent,border:`2.5px solid ${T.ink}`}}>{loked?"Lok'd ✓":"Lok"}</button>
      </div>
...
function Profile({posts,profile,setProfile,wins,lokPass,kids,cosmetics={},level,xp,quests,following,lokdInCount,bookmarks,notifications=[],notifUnread=0,loks=0,totalEarned=0,questsCompleted=0,canInstall=false,onInstall,onClearNotifs,onOpen,onDelete,onRename,say,account=null,onSignup,onLogin,pace="sweep",setPace,speed=1,setSpeed,soundLab=false,onUnlockSoundLab,soundQueue=[],setSoundQueue,founder=false,onFounderJoin,animatedToken=false}){
  ...
  const[acHandle,setAcHandle]=useState("");const[acPin,setAcPin]=useState("");const[acBusy,setAcBusy]=useState(false);
  const acGo=async mode=>{const h=acHandle.trim(),p=acPin.trim();if(h.length<2){say("Enter a handle (2+ characters)","error");return;}if(p.length<4){say("PIN needs 4+ digits","error");return;}setAcBusy(true);try{if(mode==="up")await onSignup(h,p);else await onLogin(h,p);}catch(e){const m=(e&&e.message)||"";say(m.includes("taken")?"Handle taken — try Log in instead":m.includes("nouser")?"No account with that handle":m.includes("badpin")?"Wrong PIN":"Couldn't reach the server — try again","error");}setAcBusy(false);};
    ...
    {showSettings&&(<div className="fixed inset-0 z-50 flex items-end justify-center" style={{background:"rgba(0,0,0,.35)"}} onClick={()=>setShowSettings(false)}>
      <div className="w-full rounded-t-3xl p-5" style={{maxWidth:560,background:T.card,border:`3px solid ${T.ink}`,animation:"lokrise .25s ease"}} onClick={e=>e.stopPropagation()}>
        ...
        <div className="p-3 rounded-2xl mb-2" style={{border:`3px solid ${account?T.alt:T.accent}`,background:T.paper}}>
          ...
          {account?<div className="text-xs opacity-70 mt-1 leading-snug">Signed in. Your Loks, LilLok, streak and progress back up automatically — log in on any device to pick up where you left off. Published flips live on the party feed.</div>:(<>
            <div className="text-xs opacity-70 mt-1 leading-snug">Pick a handle + a PIN (4+ digits). Your progress backs up to LokServices and your handle is your artist name on the party feed.</div>
            <div className="mt-2 flex gap-1.5">
              <input value={acHandle} onChange={e=>setAcHandle(e.target.value)} placeholder="Handle" aria-label="Account handle" className="flex-1 min-w-0 px-3 py-2 rounded-xl font-bold text-sm" style={{border:`2.5px solid ${T.ink}`,background:T.card,color:T.ink}}/>
              <input value={acPin} onChange={e=>setAcPin(e.target.value.replace(/\D/g,""))} placeholder="PIN" type="password" inputMode="numeric" maxLength={8} aria-label="Account PIN" className="px-3 py-2 rounded-xl font-bold text-sm" style={{border:`2.5px solid ${T.ink}`,background:T.card,color:T.ink,width:86}}/>
            </div>
            <div className="mt-2 flex gap-2">
              <button onClick={()=>acGo("up")} disabled={acBusy} className="lok-btn lok-display flex-1 py-2.5 rounded-xl font-extrabold text-sm" style={{background:T.accent,color:T.onAccent,border:`3px solid ${T.ink}`,opacity:acBusy?0.6:1}}>{acBusy?"…":"Create account"}</button>
              <button onClick={()=>acGo("in")} disabled={acBusy} className="lok-btn lok-display flex-1 py-2.5 rounded-xl font-extrabold text-sm" style={{background:T.card,color:T.ink,border:`3px solid ${T.ink}`,opacity:acBusy?0.6:1}}>Log in</button>
            </div>
          </>)}
        </div>
...
export default function LokApp(){
  ...
  const[account,setAccount]=useState(null);const[tips,setTips]=useState({});const[artistView,setArtistView]=useState(null);
  const adScrollTimer=useRef(null);const earnLog=useRef({ts:Date.now(),total:0});const audioCtx=useRef(null);const lastCloudPush=useRef(0);
  ...
  useEffect(()=>{(async()=>{
    ...
    const seed=[
      makeSeedLazy(drawBounce,14,"seed1",{title:"Bounce study",author:"moss.ink",paceMs:110,votes:41,voted:false,viewed:false,reactions:{splat:12,heart:30,drip:5},from:"studio",mode:"A",style:"bold",views:312}),
      makeSeedLazy(drawBloom,12,"seed2",{title:"Bloom",author:"moss.ink",paceMs:150,votes:67,voted:false,viewed:false,reactions:{splat:8,heart:52,drip:9},from:"studio",mode:"B",style:"series",views:540}),
      makeSeedLazy(drawNight,13,"seed3",{title:"Night flight",author:"moss.ink",paceMs:130,votes:29,voted:false,viewed:false,reactions:{splat:21,heart:14,drip:11},from:"studio",mode:"A",style:"bold",views:188}),
      makeSeedLazy(drawWave,14,"seed4",{title:"Harbor loop",author:"moss.ink",paceMs:130,loop:true,votes:38,voted:false,viewed:false,reactions:{splat:6,heart:27,drip:4},from:"studio",mode:"A",style:"bold",views:265}),
      makeSeedLazy(drawOrbit,16,"seed5",{title:"Orbit study",author:"inkwell_iz",paceMs:100,loop:true,votes:53,voted:false,viewed:false,reactions:{splat:9,heart:35,drip:7},from:"studio",mode:"A",style:"bold",views:410}),
      makeSeedLazy(drawWalk,12,"seed6",{title:"Little walker",author:"sketchram",paceMs:120,votes:34,voted:false,viewed:false,reactions:{splat:11,heart:19,drip:3},from:"studio",mode:"A",style:"bold",views:230}),
      makeSeedLazy(drawFish,14,"seed7",{title:"Koi drift",author:"tinta",paceMs:140,loop:true,votes:72,voted:false,viewed:false,reactions:{splat:5,heart:61,drip:12},from:"studio",mode:"A",style:"bold",views:598}),
      makeSeedLazy(drawBurst,12,"seed8",{title:"FIREWORKS!!",author:"mooncrayon",paceMs:110,votes:45,voted:false,viewed:false,reactions:{splat:30,heart:22,drip:2},from:"studio",mode:"A",style:"bold",views:340}),
      makeSeedLazy(drawRain,12,"seed9",{title:"Rain window",author:"pixel.pluto",paceMs:150,votes:26,voted:false,viewed:false,reactions:{splat:7,heart:16,drip:14},from:"studio",mode:"B",style:"series",views:172}),
    ];
  ...
  useEffect(()=>{if(!ready)return;const t=setTimeout(()=>{const blob={loks,lokPass,uiTheme,ownedThemes,effect,ownedEffects,ownedTiers,ccTier,bigBattleOwned,wins,profile,bookmarks,following,kids,customLilLok,cosmetics,owned,onboarded,sound,xp,flair,daily,quests,questsCompleted,totalEarned,traceHinted,pace,speed,soundLab,soundQueue,founder,totalSpent,fodHistory,account,tips,lillok:{...lillok,lastSeen:Date.now()}};store.set(SAVE_KEY,blob);if(account&&Date.now()-lastCloudPush.current>15000){lastCloudPush.current=Date.now();lokApi.pushSave(account,blob);}},400);return()=>clearTimeout(t);},[ready,loks,lokPass,uiTheme,ownedThemes,effect,ownedEffects,ownedTiers,ccTier,bigBattleOwned,wins,profile,bookmarks,following,kids,customLilLok,cosmetics,owned,onboarded,sound,xp,flair,daily,quests,questsCompleted,totalEarned,traceHinted,pace,speed,soundLab,soundQueue,founder,totalSpent,fodHistory,account,tips,lillok]);
  ...
  useEffect(()=>{if(!ready)return;syncFeed();const t=setInterval(()=>{if(document.visibilityState==="visible")syncFeed();},45000);return()=>clearInterval(t);},[ready,syncFeed]);
  ...
  return(<ThemeCtx.Provider value={T}>
    <div className="min-h-screen w-full" style={{background:T.paper,color:T.ink,fontFamily:"'Schibsted Grotesk',system-ui,sans-serif",animation:effect==="quake"&&!reduceMotion?"lokquake 6s infinite":"none"}}>
      ...
      {showLilLok&&<LilLokPanel lillok={lillok} phase={phase} kids={kids} custom={customLilLok} loks={loks} onFeed={feedLilLok} onFlask={()=>{if(loks<10){say("Need 10 Loks","error");return false;}setLoks(l=>l-10);setTotalSpent(t=>t+10);feedLilLok(40,"flask");say("Ink flask · −10 Loks","success");return true;}} onClose={()=>setShowLilLok(false)} say={say} setLillok={setLillok} onPublish={post=>{publishPost(post);say("Revival animation published","success");}} onSaveCustom={c=>{setCustomLilLok(c);setLillok(s=>({...s,name:c.name}));say(`${c.name} is now your LilLok`,"success");}}/>}
