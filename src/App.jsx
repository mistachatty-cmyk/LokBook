...
function Feed({posts,bookmarks,following,feedMode,setFeedMode,myHandle="",onFeatureOpen,cosmetics={},daily,streak,dailyClaimed,flipOfDay,onLine,onClaimDaily,onOpen,onVote,onLok,onBookmark,say,moodFilter,setMoodFilter,moodTags,reportedPosts,onReport,onEcho,onArtist,flair=""}){
  ...
  return(<div className="flex flex-col justify-center">
    <div className="relative mx-auto rounded-2xl overflow-hidden" style={{width:"100%",maxWidth:360,border:`3px solid ${T.ink}`,boxShadow:live?`7px 7px 0 ${T.accent}`:`6px 6px 0 ${T.shadow}`,transform:live?"scale(1)":"scale(.97)",transition:"transform .35s cubic-bezier(.22,1,.36,1), box-shadow .35s ease"}}>
      ...
      <div className="absolute left-0 right-0 bottom-0 p-3 flex items-end gap-2" style={{background:"linear-gradient(transparent, rgba(0,0,0,.6))"}}>
        ...
        <button onClick={()=>onLok(p.author||"moss.ink")} aria-label={loked?"Already Lok'd":"Lok this artist"} className="lok-btn shrink-0 px-2.5 py-1 rounded-full text-xs font-extrabold" style={{background:loked?"rgba(255,255,255,.92)":T.accent,color:loked?T.ink:T.onAccent,border:"2px solid #fff"}}>{loked?"Lok'd ✓":"Lok"}</button>
      </div>
...
function Studio({ownedTiers,ccTier,onPublish,say,kids,dailyPrompt,animFx,modules=[],legacyBrushes,setLegacyBrushes,authorName,paper="plain",cursorPack="default"}){
  ...
  return(<div className="mt-4">
    ...
    <div className="mt-2"><Easel ref={easel} key={tier} maxLayers={tier} ccTier={ccTier} modules={modules} paper={paper} cursorPack={cursorPack} onionFrames={onionFrames} animFx={animFx} legacyMode={legacyBrushes} onLegacyToggle={setLegacyBrushes}/></div>
...
function NewStudioUI({ownedTiers,ccTier,onPublish,say,kids,dailyPrompt,animFx,modules=[],legacyBrushes,setLegacyBrushes,frames,setFrames,frameDurations,setFrameDurations,title,setTitle,draftImg,setDraftImg,authorName,tutorialGhostFrames,paper="plain",cursorPack="default"}){
  const T=useT();const easel=useRef(null);
    ...
    {frames.length>0&&(<div style={{animation:"lokrise .3s ease"}}>
      <div className="mt-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={()=>{setFrames([]);setFrameDurations([]);easel.current.clearAll();setClipboardFrame(null);setDraftImg(null);say("Cleared all pages");}} aria-label="Clear all pages" className="text-[11px] font-bold underline opacity-60">clear all</button>
        </div>
      </div>
      <div className="mt-1.5 flex gap-2 overflow-x-auto pb-2">
