    ...
    {showSettings&&(<div className="fixed inset-0 z-50 flex items-end justify-center" style={{background:"rgba(0,0,0,.35)"}} onClick={()=>setShowSettings(false)}>
      <div className="w-full rounded-t-3xl p-5 overflow-y-auto overscroll-contain" style={{maxWidth:560,maxHeight:"min(85vh, 85dvh)",background:T.card,border:`3px solid ${T.ink}`,animation:"lokrise .25s ease",WebkitOverflowScrolling:"touch"}} onClick={e=>e.stopPropagation()}>
        ...
        <div className="p-3 rounded-2xl mb-2" style={{border:`2px solid ${T.shadow}`,background:T.paper}}>
          ...
          <label className="mt-2 flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={!!featureFlags.enableGyroscope} onChange={e=>{onSetFlag&&onSetFlag("enableGyroscope",e.target.checked);if(!e.target.checked){onSetFlag&&onSetFlag("gyroLoadScreen",false);onSetFlag&&onSetFlag("gyroFab",false);onSetFlag&&onSetFlag("gyroLilLokPanel",false);onSetFlag&&onSetFlag("gyroWorldMap",false);onSetFlag&&onSetFlag("gyroChestOpen",false);onSetFlag&&onSetFlag("gyroGalleryCards",false);}else{onSetFlag&&onSetFlag("gyroLoadScreen",true);onSetFlag&&onSetFlag("gyroFab",true);onSetFlag&&onSetFlag("gyroLilLokPanel",true);onSetFlag&&onSetFlag("gyroWorldMap",true);onSetFlag&&onSetFlag("gyroChestOpen",true);onSetFlag&&onSetFlag("gyroGalleryCards",true);}}} style={{accentColor:T.accent}}/> 🎮 Gyroscope (tilt interaction)</label>
