...
import { THEMES, SKIN_WAVE_GATE, SKIN_WAVE_3_GATE, SKIN_WAVE_4_GATE, ThemeCtx, useT, ART, blotBorderStyle, onColor, resolveTheme } from "./theme/theme.js";
...
import WaitlistDialog, { hasJoinedWaitlist } from "./WaitlistDialog.jsx";
...
function Viewer({posts,index,bookmarks,cosmetics={},onBookmark,onClose,onNav,onVote,onReact,onViewed,onShare,onDelete,onRename,myName="",onRemix,onPatch,onEditInStudio,gyroMotion=GYRO_ZERO}){
  ...
  // Coalesce to one state update per animation frame instead of one per raw
  // scroll event (which on a trackpad/touch can fire dozens of times a
  // frame) — was the source of the scrub feeling laggy, since each unthrottled
  // tick forced a full re-render of the flip UI even when the frame index
  ...
  const onScroll=()=>{
    ...
    requestAnimationFrame(()=>{
      ...
      const max=el.scrollHeight-el.clientHeight;const p=max>0?el.scrollTop/max:1;const idx=Math.min(n-1,Math.floor(p*n));
      setFi(f=>f===idx?f:idx);
      if(idx>=n-1)onViewed(post.id);
    });
  };
  const togglePlay=()=>{
      ...
      <div ref={scrollRef} onScroll={isB?undefined:onScroll} className="absolute inset-0 overflow-y-scroll">
        {isB?(<div className="px-3 py-4 flex flex-col items-center gap-3">{post.frames.map((f,k)=>(<div key={k} className="relative w-full" style={{maxWidth:360}}><img src={f} alt={`page ${k+1}`} loading="lazy" decoding="async" className="block w-full rounded-xl" style={{border:`4px solid ${series?T.ink:T.paper}`,boxShadow:series?"none":"0 8px 28px rgba(0,0,0,.4)"}}/><div className="absolute top-2 left-2 px-1.5 py-0.5 rounded text-xs font-bold" style={{background:series?T.ink:T.accent,color:onColor(series?T.ink:T.accent,T)}}>{String(k+1).padStart(2,"0")}</div></div>))}<div className="text-xs font-bold opacity-60 py-2" style={{color:series?T.ink:T.paper}}>↑ scroll the whole page ↑</div></div>):(
          <><div className="sticky top-0 flex items-center justify-center p-3" style={{height:"100%"}}><div className="relative" style={{maxHeight:"100%",aspectRatio:"4 / 5",transform:`rotateX(${Math.max(-12,Math.min(12,gyroMotion.beta*0.12))}deg) rotateY(${Math.max(-12,Math.min(12,gyroMotion.gamma*0.12))}deg)`,transformStyle:"preserve-3d",transition:"transform 0.1s ease-out"}}>
            <img src={post.frames[fi]} alt={`frame ${fi+1}`} decoding="async" className="block h-full w-auto max-w-full rounded-xl" style={{border:`4px solid ${series?T.ink:T.paper}`,boxShadow:series?"none":"0 12px 40px rgba(0,0,0,.45)"}}/>
...
function Studio({ownedTiers,ccTier,lokPass=false,onProUpsell,onPublish,say,kids,dailyPrompt,animFx,modules=[],legacyBrushes,setLegacyBrushes,authorName,paper="plain",cursorPack="default",mythicEquipped,stickers,onStickersChange,pendingSticker,onStickerPlaced,bloomEnabled=true,bloomIntroSeen=false,onBloomIntroSeen,grainIntroSeen=false,onGrainIntroSeen}){
...
function NewStudioUI({ownedTiers,ccTier,lokPass=false,onProUpsell,onPublish,say,kids,dailyPrompt,animFx,modules=[],legacyBrushes,setLegacyBrushes,frames,setFrames,frameDurations,setFrameDurations,title,setTitle,draftImg,setDraftImg,authorName,tutorialGhostFrames,paper="plain",cursorPack="default",postExport="png",mythicEquipped,stickers,onStickersChange,pendingSticker,onStickerPlaced,music,geoData,bloomEnabled=true,bloomIntroSeen=false,onBloomIntroSeen,grainIntroSeen=false,onGrainIntroSeen}){
  const onBloom=()=>{if(bloomEnabled&&!bloomIntroSeen){say("✨ Bright colors now glow when you capture a page — try a neon or sparkle stroke");onBloomIntroSeen&&onBloomIntroSeen();}};
  ...
  return(<div className="mt-4">
    ...
    <div className={`mt-2 ${hasCanvasBorder?"rarity-mythic":""}`} style={hasCanvasBorder?{"--mythic-bg":T.paper,borderRadius:18,padding:3}:undefined}><Easel ref={easel} key={tier} maxLayers={tier} ccTier={ccTier} lokPass={lokPass} onUpsell={onProUpsell} modules={modules} paper={paper} cursorPack={cursorPack} onionFrames={[...tutorialGhost,...onionFrames]} animFx={animFx} legacyMode={legacyBrushes} onLegacyToggle={setLegacyBrushes} stickers={stickers} onStickersChange={onStickersChange} pendingSticker={pendingSticker} onStickerPlaced={onStickerPlaced} say={say} grainIntroSeen={grainIntroSeen} onGrainIntroSeen={onGrainIntroSeen}/></div>
...
function GalleryPreview({posts,index,onClose,onNav,onWatchFull}){
  ...
  return(<div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:"rgba(0,0,0,.7)"}} onClick={onClose}>
    <div className="w-full rounded-2xl overflow-hidden" style={{maxWidth:420,background:T.card,border:`3px solid ${T.ink}`,boxShadow:`6px 6px 0 ${T.shadow}`}} onClick={e=>e.stopPropagation()}>
      ...
      <div className="p-3">
        ...
        {onWatchFull&&post.frames?.length>0&&<button onClick={()=>onWatchFull(post)} className="lok-btn lok-display mt-2.5 w-full py-2 rounded-xl text-sm font-extrabold" style={{background:T.accent,color:T.onAccent,border:`2.5px solid ${T.ink}`}}>▶ Watch full flipbook</button>}
      </div>
    </div>
  </div>);
}
...
function Profile({posts,profile,setProfile,wins,lokPass,kids,cosmetics={},level,xp,quests,following,lokdInCount,bookmarks,notifications=[],notifUnread=0,loks=0,totalEarned=0,questsCompleted=0,canInstall=false,onInstall,onClearNotifs,onOpen,onDelete,onRename,say,onCheat,pace="sweep",setPace,speed=1,setSpeed,soundLab=false,onUnlockSoundLab,soundQueue=[],setSoundQueue,founder=false,onFounderJoin,animatedToken=false,flair="",garden=[],setGarden,wordTwister={},setWordTwister,timeMachineIdx=-1,setTimeMachineIdx,heatmapData=[],sessionPin=null,setSessionPin,pinInput="",setPinInput,verified=false,setVerified,devTap,devTimer,devMode,setDevMode,appLogo,setAppLogo,hapticGrammar,setHapticGrammar,setPinUnlocked,setLoks,setTotalEarned,legacyStudio,setLegacyStudio,tutorialProgress={},onStartTutorial,viewingArtist,onBackToMyGallery,featureFlags={compactUi:false,uiScale:"normal"},onSetFlag,onRequestGyroPermission,gyroPermissionGranted=false,gyroMotion=GYRO_ZERO,weatherOverride,onSetWeather,onOpenMusic,onOpenRoadmap,onOpenWorldMap,onOpenMail,mailUnreadCount=0,onMintGuestPass,onRedeemGuestPass,onWaitlist,joinedWaitlist=false,onOpenFull}){
  ...
  return(<div>
      ...
      <div className="w-full rounded-t-3xl p-5 overflow-y-auto overscroll-contain" style={{maxWidth:560,maxHeight:"min(85vh, 85dvh)",background:T.card,border:`3px solid ${T.ink}`,animation:"lokrise .25s ease",WebkitOverflowScrolling:"touch"}} onClick={e=>e.stopPropagation()}>
        ...
        <div className="p-3 rounded-2xl mb-2" style={{border:`2px solid ${T.shadow}`,background:T.paper}}>
          ...
          {featureFlags.enableGyroscope&&(<>
            ...
            {/* Live sensor readout. There is no console on a phone, which is
                the only place tilt actually runs — without this, "gyro doesn't
                work" can't be told apart from "the sensor sends nothing". If
                these numbers move when you tilt, the sensor is fine and any
                remaining problem is in a specific effect. */}
            <div className="ml-6 mt-1.5 p-2 rounded-lg text-[10px] font-mono leading-snug" style={{background:T.shadow,color:T.ink}}>
              <div className="font-bold mb-0.5" style={{fontFamily:"inherit"}}>Live sensor — tilt your phone:</div>
              <div>tilt L/R (gamma): {(gyroMotion.gamma??0).toFixed(1)}°</div>
              <div>tilt F/B (beta): {(gyroMotion.beta??0).toFixed(1)}°</div>
              <div>compass (alpha): {(gyroMotion.alpha??0).toFixed(1)}°</div>
              <div className="mt-1 opacity-70">{(gyroMotion.gamma||gyroMotion.beta||gyroMotion.alpha)?"✓ receiving motion data":"⚠ no motion data yet — tilt the device"}</div>
            </div>
          </>)}
        ...
        <div className="p-3 rounded-2xl" style={{border:`2px solid ${T.shadow}`,background:T.paper}}>
          ...
          <div className="text-[10px] opacity-50 mt-0.5">tap a code below or type to search — one code isn't listed here</div>
          ...
          {/* "gratitude" (ILoveYouMerci) is deliberately NOT in this list. It's
              the one secret code — 100,000 Loks, only reachable by typing the
              full phrase into the box above, on purpose. Advertising it here
              the way every other code is advertised would defeat the point. */}
          <div className="max-h-32 overflow-y-auto text-xs font-bold" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px"}}>
            {[["dev mode","unlock all"],["comeback","activate"],["mercs","+200 Loks"],["overflow","max ink"],["revive","from stasis"],["new flip","fresh day"],["pin code","random"],["whisper","all voices"],["nightmode","dark mode"],["doubledown","2x Loks"],["resolve","finish quests"],["vibemode","celebration"],["debug","dev options"],["tokens","10k Loks"],["chest rain","+10 chests"],["chest spree","+18 chests"],["chest viewer","toggle"],["one chest","+1 chest"],["five chests","+5 chests"]].map(([name,desc])=>{const query=bleepCode.toLowerCase().replace(/[^a-z0-9]/g,"");const matches=query===""||name.includes(query)||desc.includes(query);return matches?<button key={name} onClick={()=>{const codes={"dev mode":"SupaBleepMode","comeback":"cincoorso","mercs":"mercmerc","overflow":"inkoverflow","revive":"phoenix","new flip":"FodSkip","pin code":"pinball","whisper":"whisper","nightmode":"nightmode","doubledown":"doubledown","resolve":"resolve","vibemode":"vibemode","debug":"CanteDev616","tokens":"Tokens10K","chest rain":"ChestRain","chest spree":"ChestSpree","chest viewer":"ChestView","one chest":"ChestOne","five chests":"ChestFive"};onCheat&&onCheat(codes[name]||name);setBleepCode("");}} className="lok-btn px-2 py-1.5 rounded-lg text-left" style={{border:`1.5px solid ${T.ink}`,background:T.card,color:T.ink,opacity:0.8}}>
              <div>{name}</div>
              <div className="text-[8px] opacity-60">{desc}</div>
            </button>:null;})}
          </div>
        </div>
          {/* Waitlist. The one permanent, always-reachable entry point — the
              guest prompt only fires after publishing or once a save has aged,
              so a lot of people would never see it there. Pre-launch this is
              the single outbound ask in the whole app. */}
          <div className="p-3 rounded-2xl mb-2" style={{border:`2.5px solid ${joinedWaitlist?T.shadow:T.accent}`,background:T.paper}}>
            <div className="font-bold text-sm">🔖 {joinedWaitlist?"You're on the waitlist":"Launching properly, later"}</div>
            <div className="text-[11px] opacity-60 mt-0.5 leading-snug">
              {joinedWaitlist
                ? "We'll email you when LokBook goes live. Until then everything you make stays on this device."
                : "Everything you draw lives on this device for now — the cloud side isn't switched on until launch. Leave an email and we'll tell you the moment it is."}
            </div>
            {!joinedWaitlist&&<button onClick={()=>onWaitlist&&onWaitlist("settings")} className="lok-btn lok-display w-full mt-2 py-2 rounded-xl font-extrabold text-sm" style={{background:T.accent,color:T.onAccent,border:`2.5px solid ${T.ink}`}}>Join the waitlist</button>}
          </div>
    ...
    {previewIdx!==null&&filtered[previewIdx]&&<GalleryPreview posts={filtered} index={previewIdx} onClose={()=>setPreviewIdx(null)} onNav={d=>setPreviewIdx(i=>Math.min(filtered.length-1,Math.max(0,i+d)))} onWatchFull={onOpenFull?post=>{setPreviewIdx(null);onOpenFull(post);}:undefined}/>}
  </div>);
}
...
export default function LokApp(){
  ...
  // Dev mode: tap-to-move-pin on the globe overrides the real geolocation
  // fix for the rest of the session, so World can be tested from anywhere
  // without spoofing the browser's actual location API.
  const[devLocationOverride,setDevLocationOverride]=useState(null);
  const userLocation=devLocationOverride||geoData.location;
  // Memoised so WorldMapViewer's data effect isn't handed a brand-new array and
  // a brand-new callback on every single App render. Passing fresh identities
  // was what kept tearing the globe down and rebuilding it — the endless
  // "loading" cycle. See the effect split in WorldMapViewer.jsx.
  const geoPosts=useMemo(()=>posts.filter(p=>p.latitude&&p.longitude),[posts]);
  const openPostById=useCallback(p=>setOpenIdx(posts.findIndex(x=>x.id===p.id)),[posts]);
  // A resident's back-catalogue (GalleryPreview) is generated on the fly and
  // never lands in the top-level `posts` array, so the real Viewer — which
  // indexes into `posts` — can't find it by id. Mirror Feed's onFeatureOpen:
  // splice the post in if it's missing, then open the real Viewer on it.
  const openPostFull=useCallback(p=>{setPosts(ps=>ps.some(x=>x.id===p.id)?ps:[...ps,p]);setTimeout(()=>setOpenIdx(i=>{const idx=posts.findIndex(x=>x.id===p.id);return idx>=0?idx:posts.length;}),0);},[posts]);
  const[loks,setLoks]=useState(260);const[myRooms,setMyRooms]=useState([]);const[pendingRoomCode,setPendingRoomCode]=useState(()=>{try{return new URLSearchParams(location.search).get("room")||null;}catch{return null;}});const[pace,setPace]=useState("sweep");const[speed,setSpeed]=useState(1);const[soundLab,setSoundLab]=useState(false);const[soundQueue,setSoundQueue]=useState([]);const[founder,setFounder]=useState(false);const[totalSpent,setTotalSpent]=useState(0);const[fodHistory,setFodHistory]=useState([]);const[lokPass,setLokPass]=useState(false);const[uiTheme,setUiTheme]=useState("riso");const[ownedThemes,setOwnedThemes]=useState(["riso"]);const[effect,setEffect]=useState("none");const[ownedEffects,setOwnedEffects]=useState(["none"]);const[ownedTiers,setOwnedTiers]=useState([10]);const[ccTier,setCcTier]=useState(false);const[bigBattleOwned,setBigBattleOwned]=useState(false);const[wins,setWins]=useState(0);const[winPulseAt,setWinPulseAt]=useState(0);
  ...
  const[appLogo,setAppLogo]=useState("default");const[devMode,setDevMode]=useState(false);/* devMode is persisted below — it used to reset on every reload, which is why the BadBleep cheat box kept disappearing. */const devTap=useRef(0);const devTimer=useRef(null);
  ...
  // Only a `dynamic` theme needs a re-resolve tick — everything else takes
  // resolveTheme's identity branch and would otherwise re-render on this timer
  // for nothing. React state changing elsewhere (gyro, toasts) already keeps
  // gyro/event-driven themes fresh enough; this covers the idle-screen case
  // for nightshift/weather/blend drivers, which depend on the wall clock.
  const[,setDynTick]=useState(0);
  useEffect(()=>{
    if(!THEMES[uiTheme]?.dynamic)return;
    const id=setInterval(()=>setDynTick(t=>t+1),15000);
    return()=>clearInterval(id);
  },[uiTheme]);
  const T=resolveTheme(THEMES[uiTheme],{nightShift:nightShiftAmount(),weather:inkWeatherToday(),isNight:nightShiftAmount()>0.4,paceMultiplier:PACE_PRESETS[pace]?.mult??1,now:Date.now()});
  const phase=lilLokPhase(lillok);const level=Math.floor(xp/100)+1;
  ...
  useEffect(()=>{(async()=>{
    ...
    if(save.comebackActive!==undefined)setComebackActive(save.comebackActive);if(save.legacyStudio!==undefined)setLegacyStudio(save.legacyStudio);if(save.legacyBrushes!==undefined)setLegacyBrushes(save.legacyBrushes);if(save.devMode!==undefined)setDevMode(save.devMode);
  ...
  // Waitlist: null when closed, otherwise the surface it was opened from, which
  // is recorded alongside the email so we can see which entry point works.
  // Pre-launch nothing but this leaves the device, so it's the only outbound
  // ask in the app and it is always an explicit tap — never auto-shown.
  const[showWaitlist,setShowWaitlist]=useState(null);
  const[joinedWaitlist,setJoinedWaitlist]=useState(hasJoinedWaitlist);
  ...
  // Every code gets its own one-line, in-universe flavor message — none of
  // them should read like a copy of another. ILoveYouMerci ("merci") in
  // particular is deliberately absent from the tap-list below (see the
  // BadBleep chip array) — it's the one secret code you have to know and
  // type yourself, so its payout and its message both stay singular.
  const onCheat=useCallback(code=>{const c=resolveCheat(code);if(!c){say("The ink doesn't recognize that…","error");return;}
    if(c.fx==="merci"){setLoks(l=>l+100000);setTotalEarned(t=>t+100000);hap([80,40,80,40,160,80,300]);blip("C6");say("💖 The ink remembers kindness · +100,000 Loks","success");pushNotif("BadBleep: a secret well-kept · +100,000 Loks","success");}
    if(c.fx==="supableep"){setLokPass(true);setOwnedThemes(Object.keys(THEMES));setOwnedEffects(EFFECTS.map(e=>e.id));setOwnedSkies(SKIES.map(x=>x.id));setModules(m=>[...new Set([...m,...STUDIO_MODULES.map(x=>x.id)])]);setOwnedAnimFx(ANIMATION_FX.map(x=>x.id));setBigBattleOwned(true);setSoundLab(true);setLoks(l=>l+9999);setTotalEarned(t=>t+9999);hap([200,80,200,80,300]);blip("C6");say("🫧 SUPABLEEP · every page of the book, unlocked","success");pushNotif("SupaBleepMode active — all content unlocked for testing","success");}
    ...
    if(c.fx==="mercmerc"){setLoks(l=>l+200);setTotalEarned(t=>t+200);hap([60,30,60]);blip("C5");say("🪙 A quiet little windfall · +200 Loks","success");}
    if(c.fx==="inkoverflow"){setLillok(s=>({...s,ink:100,bond:100,stasis:false,inkZeroAt:null,lastSeen:Date.now()}));hap([100,50,100]);say("🖋️ The well runs full — ink & bond maxed, revived!","success");}
    if(c.fx==="phoenix"){setLillok(s=>({...s,ink:Math.min(100,s.ink+50),bond:Math.min(100,s.bond+10),stasis:false,inkZeroAt:null}));hap([80,40,80]);say("🔥 Rising from grey ash, line by line","success");}
    if(c.fx==="fodskip"){setFodHistory(h=>h.filter(x=>x.day!==new Date().toDateString()));say("📖 A fresh page — Flip of the Day redrawn","success");}
    if(c.fx==="pinball"){const pin=String(Math.floor(1000+Math.random()*9000));setSessionPin(pin);say(`🔐 New PIN scratched into the margin: ${pin}`);}
    if(c.fx==="whisper"){setOwned(o=>({...o,voicePack:VOICE_PACKS.map(v=>v.id)}));setCosmetics(c=>({...c,voicePack:"whisper"}));say("🗣️ Every voice pack, unhushed","success");}
    if(c.fx==="nightmode"){if(ownedThemes.includes("midnight"))setUiTheme("midnight");else if(ownedThemes.includes("noir"))setUiTheme("noir");else{setOwnedThemes(t=>[...new Set([...t,"midnight"])]);setUiTheme("midnight");}say("🌙 The page dims — night mode drawn on","success");}
    if(c.fx==="doubledown"){setLoks(l=>l*2);setTotalEarned(t=>t*2);hap([100,50,100]);blip("C6");say("✨ Double Down — every Lok, doubled","success");}
    if(c.fx==="resolve"){setQuests(q=>{if(!q)return q;let paid=0;const items=q.items.map(it=>{if(it.done)return it;paid+=it.reward;return{...it,progress:it.goal,done:true};});if(paid){setLoks(l=>l+paid);setTotalEarned(t=>t+paid);setTimeout(()=>say(`📋 Every quest closed out · +${paid} Loks`,"success"),250);}return{...q,items};});}
    if(c.fx==="vibemode"){const ids=CELEBRATIONS.map(x=>x.id);const idx=(ids.indexOf(celebrationStyle)+1)%ids.length;setCelebrationStyle(ids[idx]);say(`🎉 New celebration style drawn up: ${ids[idx]}!`);}
    if(c.fx==="devmode"){setDevMode(true);hap([80,40,80,40,160]);blip("C6");say("🔧 The margins open — Dev Mode unlocked","success");pushNotif("Developer mode active — debugging tools and secret features available","success");}
    if(c.fx==="tokens10k"){setLoks(l=>l+10000);setTotalEarned(t=>t+10000);hap([200,100,200,100,300]);blip("C6");say("🪙 An emergency stash, uncovered · +10,000 Loks","success");pushNotif("BadBleep: emergency tokens · +10,000 Loks","success");}
    if(c.fx==="chestrain"){const newChests=Array.from({length:10},()=>({id:`c${Date.now()}-${Math.random()}`,type:["common","uncommon","rare","epic","legendary","mythic"][Math.floor(Math.random()*6)]}));setChests(prev=>[...prev,...newChests]);hap([50,30,50]);say("🎁 A downpour of 10 chests — for testing","success");}
    if(c.fx==="chestspree"){const rarities=["common","uncommon","rare","epic","legendary","mythic"];const newChests=rarities.flatMap(r=>Array.from({length:3},()=>({id:`c${Date.now()}-${Math.random()}`,type:r})));setChests(prev=>[...prev,...newChests]);hap([100,50,100,50,150]);say("🎁 Chest Spree! One of every rarity, three times over — for testing","success");}
    if(c.fx==="chestview"){setShowChestViewer(prev=>!prev);}
    if(c.fx==="chest1"){const newChests=[{id:`c${Date.now()}-${Math.random()}`,type:["common","uncommon","rare","epic","legendary","mythic"][Math.floor(Math.random()*6)]}];setChests(prev=>[...prev,...newChests]);hap([30,20,30]);say("🎁 One chest, dropped in","success");}
    if(c.fx==="chest5"){const newChests=Array.from({length:5},()=>({id:`c${Date.now()}-${Math.random()}`,type:["common","uncommon","rare","epic","legendary","mythic"][Math.floor(Math.random()*6)]}));setChests(prev=>[...prev,...newChests]);hap([50,30,50,30,50]);say("🎁 Five chests, stacked up","success");}
  ...
  // Buy-or-equip for the categories whose ownership lives in `owned[cat]` but
  // whose EQUIPPED value is separate top-level state (fontPack, cursorPack,
  // stickerPack). These three had no ownership check at all — not the wrong
  // shape, none — so they called spend() unconditionally. The Shop renders an
  // owned card labelled "Equip", ShopItem skips its confirm tap precisely
  // BECAUSE you own it, and a single tap silently charged full price again.
  // Switching between two font packs you already owned billed you every time.
  // It is the same double-charge the ownsCosmetic helpers were introduced to
  // kill, surviving in the three categories never converted to onBuyCosmetic.
  //
  // The raw `[...(o.cat||[]), id]` spread was the second half of it: it skips
  // ownedIds() normalisation, so a legacy {id,ts} save grew a duplicate entry
  // per tap and could hold both shapes at once.
  const buyEquip=(cat,id,item,setEquipped)=>{
    if(ownsCosmetic(owned,cat,id)){setEquipped(id);blip("D5");say(`Equipped ${item.name}`);return;}
    spend(item.price,()=>{setOwned(o=>({...o,[cat]:[...ownedIds(o,cat),id]}));setEquipped(id);blip("C6");},`${item.name} unlocked`);
  };
  ...
  return(<ThemeCtx.Provider value={T}>
    ...
    <div className={`min-h-screen w-full lok-motion-${featureFlags.lokMotion||"subtle"} ${featureFlags.compactUi ? "lok-compact" : ""}`} style={{background:T.paper,color:T.ink,fontFamily:resolveFont(fontPack),...(isUnlocked("night_shift",level)&&nightShiftAmount()>0.02?{filter:`brightness(${1-nightShiftAmount()*0.16}) saturate(${1-nightShiftAmount()*0.22}) hue-rotate(${-nightShiftAmount()*8}deg)`,transition:"filter 4s linear"}:{}),animation:effect==="quake"&&!reduceMotion?"lokquake 6s infinite":"none"}}>
      <GlobalStyle T={T} pace={pace} speed={speed}/><ThemeBackdrop themeId={uiTheme} pace={pace} gyroMotion={featureFlags.enableGyroscope?gyroMotion:GYRO_ZERO} enableGyroscope={featureFlags.enableGyroscope} winIntensity={Math.max(0,1-(Date.now()-winPulseAt)/30000)}/><SkyEffect sky={sky} paper={T.paper}/><PageEffect effect={effect}/>{/* Ink Weather (roadmap, LV2): a second, unbought effect layer the day picks for you */}
      ...
      <main className="mx-auto w-full px-4" style={{maxWidth:560,paddingBottom:"calc(160px + env(safe-area-inset-bottom))"}}>
        ...
        {stickerSheetOpen&&<StickerSheet T={T} say={say} equippedPack={stickerPack} myStickerIds={myStickers} onClose={()=>setStickerSheetOpen(false)} onPick={item=>{setPendingSticker(item);setStickerSheetOpen(false);say("Tap the canvas to place it");}} onUploaded={id=>refreshMyStickers()} onDeleteMy={id=>{deleteSticker(id).then(refreshMyStickers);}}/>}
          ...
          {featureFlags.combatUIMode!=="legacy"&&(tab==="battle"||tab==="front")&&<CombatHub
            ...
            onLine={showLine} onUnlockBig={()=>{if(bigBattleOwned){say("Big Battle is already unlocked");return;}spend(50,()=>setBigBattleOwned(true),"Big Battle unlocked");}}
            onResult={(won,mult=1)=>{addLoks((won?25:5)*mult);gainXp(won?25:8);questTick("battle");if(won){setWins(w=>w+1);setWinPulseAt(Date.now());hap([200,100,200]);pushNotif(`You won a battle! +${25*mult} Loks${mult>1?" · ✦ 3× featured":""}`,"success");feedLilLok(5,"creation");}setLillok(s=>s.stasis?s:({...s,ink:Math.max(0,s.ink-6)}));}}
          ...
          {tab==="shop"&&<Shop ccTier={ccTier} say={say} modules={modules} onBuyModule={m=>{if(modules.includes(m.id)){say("Already owned");return;}spend(m.price,()=>{setModules(o=>[...o,m.id]);blip("C6");},`${m.name} unlocked`);}} loks={loks} lokPass={lokPass} kids={kids} uiTheme={uiTheme} ownedThemes={ownedThemes} effect={effect} ownedEffects={ownedEffects} sky={sky} ownedSkies={ownedSkies} onSky={(id,s)=>{if(ownedSkies.includes(id)){setSky(id);say(`Equipped ${s.name}`);}else spend(s.price,()=>{setOwnedSkies(o=>[...o,id]);setSky(id);},`${s.name} unlocked`);}} animFx={animFx} ownedAnimFx={ownedAnimFx} onAnimFx={(id,f)=>{if(ownedAnimFx.includes(id)){setAnimFx(id);say(id==="none"?"FX off":`${f.name} equipped`);}else spend(f.price,()=>{setOwnedAnimFx(o=>[...o,id]);setAnimFx(id);},`${f.name} unlocked`);}} fontPack={fontPack} onFontPack={(id,f)=>buyEquip("fontPack",id,f,setFontPack)} cursorPack={cursorPack} onCursorPack={(id,c)=>buyEquip("cursorPack",id,c,setCursorPack)} musicPack={musicPack} stickerPack={stickerPack} onStickerPack={(id,s)=>buyEquip("stickerPack",id,s,setStickerPack)} postExport={postExport} mythicOwned={mythicOwned} mythicEquipped={mythicEquipped} dailyOwned={dailyOwned} weeklyOwned={weeklyOwned} onBuyMythic={(item,rotation)=>{if(rotation==="daily"||rotation==="weekly"){if(isArchivedRotation(item)){say(`${item.name} isn't active yet — it needs a real encoder, not just wiring`);return;}const ownedIds=rotation==="daily"?dailyOwned:weeklyOwned;if(ownedIds.includes(item.id)){applyRotation(item);say(`Equipped ${item.name}`);return;}spend(item.price,()=>{rotation==="daily"?setDailyOwned(o=>[...o,item.id]):setWeeklyOwned(o=>[...o,item.id]);applyRotation(item);blip("C6");},`${item.name} unlocked`);}else{if(mythicOwned.includes(item.id)){setMythicEquipped(item.id);say(`Equipped ${item.name}`);}else spend(item.price,()=>{setMythicOwned(o=>[...o,item.id]);setMythicEquipped(item.id);setTimeout(()=>setCelebration(item.name),100);setTimeout(()=>setCelebration(null),3000);},`${item.name} unlocked`);}}} cosmetics={cosmetics} owned={owned} setKids={setKids} onBuyCosmetic={(cat,item)=>{if(ownsCosmetic(owned,cat,item.id)){setCosmetics(c=>({...c,[cat]:item.id}));blip("D5");say(`Equipped ${item.name}`);if(cat==="blotPersonality")setBlotSpeech(getBlotResponse(item.id,"greet",lillok.name));}else spend(item.price,()=>{setOwned(o=>({...o,[cat]:[...ownedIds(o,cat),item.id]}));setCosmetics(c=>({...c,[cat]:item.id}));blip("C6");setBlotSpeech(getBlotResponse(cat==="blotPersonality"?item.id:cosmetics.blotPersonality,"purchase",lillok.name));},`${item.name} unlocked`);}} onBuyPass={()=>{setLokPass(true);setOwnedThemes(Object.keys(THEMES));blip("C6");say("LokPass active!");}} onTheme={id=>{if(ownedThemes.includes(id)){setUiTheme(id);say(`Equipped ${THEMES[id].name}`);}else spend(THEMES[id].price,()=>{setOwnedThemes(o=>[...o,id]);setUiTheme(id);},`${THEMES[id].name} unlocked`);}} onEffect={(id,e)=>{if(ownedEffects.includes(id)){setEffect(id);say(id==="none"?"Effects off":`${e.name} equipped`);}else spend(e.price,()=>{setOwnedEffects(o=>[...o,id]);setEffect(id);},`${e.name} unlocked`);}} onCc={()=>{if(ccTier){say("Studio Pro is already yours");return;}spend(120,()=>setCcTier(true),"Studio Pro unlocked");}} ownedTiers={ownedTiers} onBuyTier={t=>{if(ownedTiers.includes(t.layers)){say(`${t.label} is already unlocked — pick it in Studio`);return;}spend(t.price,()=>{setOwnedTiers(o=>[...o,t.layers]);blip("C6");},`${t.label} unlocked`);}} celebrationStyle={celebrationStyle} onCelebrationStyle={id=>setCelebrationStyle(id)} enableGyroscope={featureFlags.enableGyroscope}/>}
        </Suspense>
        </div>
      </main>
      ...
      {showWorldMap&&<ErrorBoundary compact><Suspense fallback={<div style={{position:"fixed",inset:0,zIndex:50,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.8)",color:"#fff"}}>Loading map…</div>}><WorldMapViewer posts={geoPosts} userLocation={userLocation} viewerId={auth.getUserId()||profile.name} theme={uiTheme} skin={cosmetics.globeSkin} gyroMotion={featureFlags.enableGyroscope&&featureFlags.gyroWorldMap?gyroMotion:GYRO_ZERO} onPostClick={openPostById} onClose={()=>setShowWorldMap(false)} devMode={devMode} onLocationOverride={loc=>{setDevLocationOverride(loc);say(`📍 Dev pin moved to ${loc.lat.toFixed(3)}, ${loc.lng.toFixed(3)}`);}}/></Suspense></ErrorBoundary>}
      ...
      {showGuestPrompt&&<GuestSavePrompt reason={showGuestPrompt} guestDays={featureFlags.guestReminderDays||7} onSignIn={auth.signInWithEmail} onMint={mintGuestPassCode} onSetDays={n=>{setFeatureFlags(f=>({...f,guestReminderDays:n}));store.set("lok:flags",{...featureFlags,guestReminderDays:n});}} onClose={()=>setShowGuestPrompt(null)} onWaitlist={()=>{setShowGuestPrompt(null);setShowWaitlist("guest-prompt");}} joinedWaitlist={joinedWaitlist}/>}
      {showWaitlist&&<WaitlistDialog source={showWaitlist} onClose={()=>{setJoinedWaitlist(hasJoinedWaitlist());setShowWaitlist(null);}}/>}
