  ...
  return(<div>
    ...
    {list.length===0?(
      ...
      <div ref={wrapRef} onScroll={onScroll} className="mt-3 -mx-4" style={{height:"calc(100dvh - 300px)",minHeight:360,overflowY:"scroll",scrollSnapType:"y mandatory"}}>
        {list.map((p,i)=>(<FeedCard key={p.id} p={p} live={i===active} marked={bookmarks.includes(p.id)} loked={following.includes(p.author||"moss.ink")} cosmetics={cosmetics} onOpen={onOpen} onVote={onVote} onLok={onLok} onBookmark={onBookmark} moodTags={moodTags} onReport={onReport} onEcho={onEcho} flair={flair}/>))}
      </div>
    )}</>)}
  </div>);
}
  ...
  return(<div className="px-4 flex flex-col justify-center" style={{height:"100%",scrollSnapAlign:"start"}}>
    <div className="relative mx-auto rounded-2xl overflow-hidden" style={{width:"100%",maxWidth:360,border:`3px solid ${T.ink}`,boxShadow:live?`7px 7px 0 ${T.accent}`:`6px 6px 0 ${T.shadow}`,transform:live?"scale(1)":"scale(.97)",transition:"transform .35s cubic-bezier(.22,1,.36,1), box-shadow .35s ease"}}>
      ...
      <div className="absolute left-0 right-0 bottom-0 p-3 flex items-end gap-2" style={{background:"linear-gradient(transparent, rgba(0,0,0,.6))"}}>
        <div className="flex-1 text-white min-w-0"><div className="lok-display font-extrabold leading-tight truncate">{p.title}</div><div className="text-xs opacity-90"><NameTag name={p.author||"moss.ink"} color={cosmetics.nameColor} style={{color:"#fff"}}/>{flair?<span className="ml-1.5 text-[10px] font-bold tracking-wide" style={{color:"#F0DB4F"}}>{flair}</span>:null} · {p.from==="revival"?"revival loop":p.from==="battle"?"battle piece":p.mode==="B"?"page-flip":"flipbook"}</div></div>
...
const Easel=forwardRef(function Easel({maxLayers,ccTier,onionFrames=[],onStroke,animFx="none"},ref){
  ...
  const freehandPts=useRef(null);const freehandBaseline=useRef(null);
  ...
  const useFreehand=()=>brush==="ink"&&tool==="pen"&&symmetry==="none";
  const drawFreehandStroke=ctx=>{if(!freehandPts.current||!freehandBaseline.current)return;ctx.putImageData(freehandBaseline.current,0,0);const outline=getStroke(freehandPts.current,{size,thinning:0.5,smoothing:0.5,streamline:0.5,simulatePressure:freehandPts.current._simulate});if(!outline.length)return;ctx.globalCompositeOperation="source-over";ctx.fillStyle=color;ctx.globalAlpha=1;ctx.fill(new Path2D(getSvgPathFromStroke(outline)));};
  ...
  const stamp=(ctx,x,y,start)=>{
    ...
    if(brush==="custom"){pts.forEach(([sx,sy])=>dabCustom(ctx,sx,sy,es,color,customBrushParams));return;}
  ...
  const down=e=>{e.preventDefault();const cv=canvases.current.get(active);if(!cv||!activeLayer?.visible)return;e.currentTarget.setPointerCapture(e.pointerId);const p0=pos(e);sizeMulRef.current=dynMul(e,p0[0],p0[1]);if(tool==="eyedrop"){eyedrop(...p0);return;}if(tool==="transform"){transformDrag.current={startClient:[e.clientX,e.clientY],startCanvas:p0};return;}pushUndo();if(tool==="fill"){fillLayer(cv.getContext("2d"));return;}drawing.current=true;onStroke&&onStroke();const ctx=cv.getContext("2d");if(useFreehand()){const pr=e.pointerType==="pen"&&e.pressure>0?e.pressure:0.5;freehandPts.current=[[p0[0],p0[1],pr]];freehandPts.current._simulate=e.pointerType!=="pen";freehandBaseline.current=ctx.getImageData(0,0,W,H);drawFreehandStroke(ctx);return;}stamp(ctx,...p0,true);};
  const move=e=>{if(tool==="transform"){if(!transformDrag.current)return;const cv=canvases.current.get(active);if(cv)cv.style.transform=`translate(${e.clientX-transformDrag.current.startClient[0]}px,${e.clientY-transformDrag.current.startClient[1]}px)`;return;}if(!drawing.current)return;const cv=canvases.current.get(active);if(!cv)return;const evs=(e.getCoalescedEvents&&e.getCoalescedEvents().length)?e.getCoalescedEvents():[e];const ctx=cv.getContext("2d");if(freehandPts.current){evs.forEach(ev=>{const p=pos(ev);const pr=ev.pointerType==="pen"&&ev.pressure>0?ev.pressure:0.5;freehandPts.current.push([p[0],p[1],pr]);});drawFreehandStroke(ctx);return;}const p=pos(evs[evs.length-1]);sizeMulRef.current=dynMul(e,p[0],p[1]);evs.forEach(ev=>stamp(ctx,...pos(ev),false));};
...
function NewStudioUI({ownedTiers,ccTier,onPublish,say,kids,dailyPrompt,animFx,modules=[],legacyBrushes,setLegacyBrushes,frames,setFrames,frameDurations,setFrameDurations,title,setTitle,draftImg,setDraftImg,authorName,tutorialGhostFrames}){
  ...
  return(<div className="mt-4">
    ...
    <div className="mt-2"><Easel ref={easel} key={tier} maxLayers={tier} ccTier={ccTier} modules={modules} onionFrames={[...tutorialGhost,...onionFrames]} animFx={animFx} legacyMode={legacyBrushes} onLegacyToggle={setLegacyBrushes}/></div>
