...
import Profile, { Onboard, ArtistPage } from "./pages/Profile.jsx";
...
export default function LokApp(){
  ...
  useEffect(()=>{if(!ready)return;const t=setTimeout(()=>{const blob={loks,lokPass,uiTheme,ownedThemes,effect,ownedEffects,sky,ownedSkies,studioModules,bigBattleOwned,wins,offlineBonusDay,tides,eventClaimed,profile,bookmarks,following,kids,customLilLok,cosmetics,owned,onboarded,sound,xp,flair,daily,quests,questsCompleted,totalEarned,traceHinted,pace,speed,soundLab,soundQueue,founder,totalSpent,fodHistory,account,tips,compactDensity,lillok:{...lillok,lastSeen:Date.now()}};store.set(SAVE_KEY,blob);if(account&&Date.now()-lastCloudPush.current>15000){lastCloudPush.current=Date.now();lokApi.pushSave(account,blob,auth.getUserId());}},400);return()=>clearTimeout(t);},[ready,loks,lokPass,uiTheme,ownedThemes,effect,ownedEffects,sky,ownedSkies,studioModules,bigBattleOwned,wins,offlineBonusDay,tides,eventClaimed,profile,bookmarks,following,kids,customLilLok,cosmetics,owned,onboarded,sound,xp,flair,daily,quests,questsCompleted,totalEarned,traceHinted,pace,speed,soundLab,soundQueue,founder,totalSpent,fodHistory,account,tips,compactDensity,lillok]);
