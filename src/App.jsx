...
import {
  ...
  RARITY, MYTHIC_ITEMS, CELEBRATIONS, getDailyRotation, getWeeklyRotation, fromDbPost
} from "./constants.jsx";
...
export default function LokApp(){
  ...
  useEffect(()=>{(async()=>{
    ...
    if(save.comebackActive!==undefined)setComebackActive(save.comebackActive);
    ...
    (async()=>{try{const{data,error}=await supabase.from("lok_posts").select("id,title,frames,pace_ms,mode,style,author,votes,views").order("votes",{ascending:false}).limit(6);if(!error&&data){const dbPosts=data.map(fromDbPost).filter(Boolean).map(p=>({...p,voted:false,viewed:false}));setPosts(ps=>{const existing=new Set(ps.map(x=>x.id));return[...ps,...dbPosts.filter(dp=>!existing.has(dp.id))];});}}catch{}})().finally(()=>{const elapsed=Date.now()-loadStart.current;setTimeout(()=>setReady(true),Math.max(0,3500-elapsed));});
