...
function Battle({ownedTiers,ccTier,wins,bigBattleOwned,kids,phase,lillok,customLilLok,onResult,onUnlockBig,onPublish,onLine,blip,hap,say,animFx,authorName,modules=[],paper="plain",cursorPack="default",bloomEnabled=true,bloomIntroSeen=false,onBloomIntroSeen,grainIntroSeen=false,onGrainIntroSeen}){
  const onBloom=()=>{if(bloomEnabled&&!bloomIntroSeen){say("✨ Bright colors now glow when you capture a page — try a neon or sparkle stroke");onBloomIntroSeen&&onBloomIntroSeen();}};
  const T=useT();
