...
| A13 | Add `npm run smoke` (and ideally `npm run build`) as a CI check (GitHub Actions) so the gate is enforced automatically, not just by convention | ⬜ | Currently manual discipline only |
...
| D3 | `lok_live.jsx` at repo root — confirmed dead, never edit, candidate for deletion once everyone's certain nothing references it | ⬜ | Low priority, low risk either way |
