...
| A12 | Audit remaining Shop categories (cursors, fonts, music, export, stickers, mythic/daily/weekly rotations) for sold-but-broken items the same way A4-A7 covered | ✅ | Done — `docs/AUDIT.md`. Cursors/fonts/music/export/stickers were already covered by `INERT_FEATURES.md` §3 (this row was stale). The rotations were the real gap: **95 items sold and inert** — `DAILY_ITEMS` (49) and `WEEKLY_ITEMS` (25) have zero consumers outside Shop; `MYTHIC_ITEMS` (21) only render a badge + one `canvas_border`, and `mythicEquipped` isn't even passed to `NewStudioUI`, so they're invisible unless Legacy Studio is on. Also found `scripts/audit-inert.mjs` reports **false INERT** for `stickerPack`/`postExport` (it only greps `cosmetics.<key>`; those are top-level state) — regenerating the doc from it would inject wrong claims |
| A17 | `npm run smoke` was failing on a clean `master` (`useAuth must be used within AuthProvider`) — the mandatory §2.3 gate was red, meaning it was being skipped | ✅ | `scripts/smoke.mjs` rendered `App` bare; `main.jsx` mounts it inside `<AuthProvider>`. Harness now wraps it the same way. **A red gate is the §1 precondition — treat this as urgent if it ever recurs.** |
