...
-   ~~**Dev-mode "global songs"**~~: Done, as a small local-only step toward this — any track can be starred "Global" from the Dev panel (7-tap `devMode` gate) and stays in the queue regardless of active playlist. This is **not** the real curated-artist-pack system below: it's per-device, not synced to other users, and has no admin/backend involved.
-   **Curated artist packs** (admin-only, real version): a way for the LokBook team (not average users) to add official playlists from consenting artists, synced to *every* user — a named pack with one shared cover image and an attribution link (Instagram/website/Linktree) applied to every track in it. Needs: a Supabase table (or storage bucket) for curated tracks/cover art, RLS so only an admin/dev-gated account can write, and an admin UI. Not started — this is real backend work (schema + RLS + storage bucket + admin UI), not a client-only feature like the dev-mode toggle above.
